...
export default function ListProjectsAnimate({ projects, onProjectClick }: ProjectMarqueeProps) {
  const { t } = useLanguage();
        ...
        {[...projects, ...projects, ...projects, ...projects].map((project, idx) => {
          ...
          return (
            <div
              ...
              className="w-[320px] shrink-0 bg-gray-50 dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-gray-900 flex flex-col justify-between hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-300 group shadow-sm cursor-pointer"
            >
              <div>
